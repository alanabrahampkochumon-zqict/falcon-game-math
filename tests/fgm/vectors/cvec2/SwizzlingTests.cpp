 ...
 *
 * @brief Verify @ref fgm::CVec2 swizzling logic.
 *
...
/**
 * @addtogroup T_FGM_CVec2_Swizzle
 * @{
 */
...
namespace
{
    ...
    /**
     * @brief Test fixture for @ref fgm::CVec2 swizzling.
     */
    template <typename>
    class CVec2SwizzlingTests: public testing::Test
    {};
    TYPED_TEST_SUITE(CVec2SwizzlingTests, SupportedArithmeticTypes);
    ...
    {
        constexpr fgm::CVec2 VECTOR(1.0f, 2.0f);
        ...
        static_assert(fgm::CVec2<float>::swizzle<X, Y>(VECTOR).x() == 1.0f);
        static_assert(fgm::CVec2<float>::swizzle<X, Y>(VECTOR).y() == 2.0f);

        /// @test Verify that vector swizzle<x,x>(static wrapper) is available at compile time.
        static_assert(fgm::CVec2<float>::swizzle<X, X>(VECTOR).x() == 1.0f);
        static_assert(fgm::CVec2<float>::swizzle<X, X>(VECTOR).y() == 1.0f);

        /// @test Verify that vector swizzle<y,y>(static wrapper) is available at compile time.
        static_assert(fgm::CVec2<float>::swizzle<Y, Y>(VECTOR).x() == 2.0f);
        static_assert(fgm::CVec2<float>::swizzle<Y, Y>(VECTOR).y() == 2.0f);

        /// @test Verify that vector swizzle<y,x>(static wrapper) is available at compile time.
        static_assert(fgm::CVec2<float>::swizzle<Y, X>(VECTOR).x() == 2.0f);
        static_assert(fgm::CVec2<float>::swizzle<Y, X>(VECTOR).y() == 1.0f);

        /// @test Verify that vector swizzle<x>(static wrapper) is available at compile time.
        static_assert(fgm::CVec2<float>::swizzle<X>(VECTOR) == 1.0f);

        /// @test Verify that vector swizzle<y>(static wrapper) is available at compile time.
        static_assert(fgm::CVec2<float>::swizzle<Y>(VECTOR) == 2.0f);


        /// @test Verify that vector swizzle<r, g>(static wrapper) is available at compile time.
        static_assert(fgm::CVec2<float>::swizzle<R, G>(VECTOR).r() == 1.0f);
        static_assert(fgm::CVec2<float>::swizzle<R, G>(VECTOR).g() == 2.0f);

        /// @test Verify that vector swizzle<g, r>(static wrapper) is available at compile time.
        static_assert(fgm::CVec2<float>::swizzle<G, R>(VECTOR).r() == 2.0f);
        static_assert(fgm::CVec2<float>::swizzle<G, R>(VECTOR).g() == 1.0f);


        /// @test Verify that vector swizzle<s, t>(static wrapper) is available at compile time.
        static_assert(fgm::CVec2<float>::swizzle<S, T>(VECTOR).s() == 1.0f);
        static_assert(fgm::CVec2<float>::swizzle<S, T>(VECTOR).t() == 2.0f);

        /// @test Verify that vector swizzle<t, s>(static wrapper) is available at compile time.
        static_assert(fgm::CVec2<float>::swizzle<T, S>(VECTOR).s() == 2.0f);
        static_assert(fgm::CVec2<float>::swizzle<T, S>(VECTOR).t() == 1.0f);


        /// @test Verify that vector swizzle<s>(static wrapper) is available at compile time.
        static_assert(fgm::CVec2<float>::swizzle<S>(VECTOR) == 1.0f);

        /// @test Verify that vector swizzle<t>(static wrapper) is available at compile time.
        static_assert(fgm::CVec2<float>::swizzle<T>(VECTOR) == 2.0f);
...
TYPED_TEST(CVec2SwizzlingTests, SwizzlingWorksAcrossDifferentTypes)
{
    const fgm::CVec2 vector(TypeParam(1), TypeParam(2));
    const fgm::CVec2 expectedSwizzling(TypeParam(2), TypeParam(1));
...
TEST(CVec2SwizzlingTest, SwizzlingWorksForBooleanVector)
{
    const fgm::CVec2 vector(true, false);
    const fgm::CVec2 expectedSwizzling(false, true);
...
/**
 * @test Verify that swizzling using static variant of @ref fgm::CVec2::swizzle
 *        returns shuffled vector across different numeric.
 */
TYPED_TEST(CVec2SwizzlingTests, StaticWrapper_SwizzlingWorksAcrossDifferentTypes)
{
    const fgm::CVec2 vector(TypeParam(1), TypeParam(2));
    const fgm::CVec2 expectedSwizzling(TypeParam(2), TypeParam(1));

    const auto swizzledVector = fgm::CVec2<TypeParam>::template swizzle<Y, X>(vector);
 ...
 */
TEST(CVec2SwizzlingTest, StaticWrapper_SwizzlingWorksForBooleanVector)
{
    const fgm::CVec2 vector(true, false);
    const fgm::CVec2 expectedSwizzling(false, true);

    const auto swizzledVector = fgm::CVec2<bool>::template swizzle<Y, X>(vector);
