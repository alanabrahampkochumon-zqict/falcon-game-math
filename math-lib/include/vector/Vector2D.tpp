...
namespace fgm {
    ...
    constexpr PromotedValue_t<T, U> Vector2D<T>::cross(const Vector2D<U> &rhs) const noexcept requires
    ...
    template<Arithmetic T>
    template<SignedStrictArithmetic U>
    constexpr PromotedValue_t<T, U> Vector2D<T>::cross(const Vector2D &lhs, const Vector2D<U> &rhs) noexcept requires
        SignedStrictArithmetic<T> {
