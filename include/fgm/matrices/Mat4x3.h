 ...
 *
 * @brief Templated 4x3 Matrix supporting integral, floating-point, and boolean types.
 *
 * @details Provide high-performance 4x3 matrix implementation with SIMD acceleration
...
namespace fgm
{
    ...
    struct Mat4x3
    {
        ...
        /**
         * @brief Initialize an uninitialized @ref Mat4x3 instance.
         *
        ...
        /**
         * @brief Initialize a 4x3 matrix from the passed-in scalar elements.
         *
         ...
         *
         */
        [[nodiscard]] constexpr Mat4x3(T m00, T m01, T m02, T m10, T m11, T m12, T m20, T m21, T m22, T m30, T m31,
                                       T m32) noexcept;


        /**
         * @brief Initialize a 4x3 matrix from the passed-in vectors as columns.
         *
         * @param[in] col0 The 4x3-vector to use as the first column entry.
         * @param[in] col1 The 4x3-vector to use as the second column entry.
         * @param[in] col2 The 4x3-vector to use as the third column entry.
         */
        [[nodiscard]] constexpr Mat4x3(const Vec4<T>& col0, const Vec4<T>& col1, const Vec4<T>& col2) noexcept;



        /**
         * @brief Initialize @ref Mat4x3 from another @ref Mat4x3 of a different type.
         *
        ...
        [[nodiscard]] explicit constexpr Mat4x3(const Mat4x3<U>& other) noexcept;
         ...
         */
        [[nodiscard]] constexpr Vec4<T>& operator[](std::size_t col) noexcept;
         ...
         */
        [[nodiscard]] constexpr const Vec4<T>& operator[](std::size_t col) const noexcept;
        ...
        [[nodiscard]] constexpr bool allEq(const Mat4x3<U>& rhs,
        ...
        template <Arithmetic U>
            requires StrictSignedness<T, U>
        [[nodiscard]] static constexpr bool allEq(const Mat4x3& lhs, const Mat4x3<U>& rhs,
        ...
        template <Arithmetic U>
            requires StrictSignedness<T, U>
        [[nodiscard]] constexpr bool anyNeq(const Mat4x3<U>& rhs,
                                            double epsilon = std::is_same_v<T, double> || std::is_same_v<U, double>
                                                ? Config::DOUBLE_EPSILON
                                                : Config::FLOAT_EPSILON) const noexcept;
        ...
        template <Arithmetic U>
            requires StrictSignedness<T, U>
        [[nodiscard]] static constexpr bool anyNeq(const Mat4x3& lhs, const Mat4x3<U>& rhs,
                                                   double epsilon = std::is_same_v<T, double> ||
                                                           std::is_same_v<U, double>
                                                       ? Config::DOUBLE_EPSILON
                                                       : Config::FLOAT_EPSILON) noexcept;
        ...
        /**
         * @copybrief allEq(const Mat4x3<U>&, double) const noexcept
         *
         * @note Implements an explicit constexpr MSVC workaround to ensure IEEE 754 NaN compliance
         *       during static evaluation.
         *
        ...
        template <Arithmetic U>
            requires StrictSignedness<T, U>
        [[nodiscard]] constexpr bool operator==(const Mat4x3<U>& rhs) const noexcept;


        /**
         * @copybrief anyNeq(const Mat4x3<U>&, double) const noexcept
         *
         * @note Implements an explicit constexpr MSVC workaround to ensure IEEE 754 NaN compliance
         *       during static evaluation.
         *
         ...
         *
         * @return A new @ref Mat4x3 containing the element-wise sum.
         */
        ...
        template <StrictArithmetic U>
            requires StrictSignedness<T, U>
        Mat4x3& operator+=(const Mat4x3<U>& rhs) noexcept
            requires StrictArithmetic<T>;
        ...
        [[nodiscard]] constexpr PromotedMat4x3<T, S> operator*(S scalar) const noexcept
            requires StrictArithmetic<T>;
         ...
         */
        template <StrictArithmetic S>
        constexpr Mat4x3& operator*=(S scalar) noexcept
            requires StrictArithmetic<T>;
         ...
         * @param[in] scalar The value to scale by.
         *
         * @return A new @ref Mat4x3 inverse scaled by @p scalar.
         */
        template <StrictArithmetic S>
        [[nodiscard]] constexpr PromotedMat4x3<T, S> operator/(const S& scalar) const noexcept
            requires StrictArithmetic<T>;
         ...
         */
        template <StrictArithmetic S>
        constexpr Mat4x3& operator/=(const S& scalar) noexcept
            requires StrictArithmetic<T>;
         ...
         *                     element.
         *
         * @return A new @ref Mat4x3 resulting from the division or @p fallback if the @p scalar is below the
         *         epsilon threshold or if the matrix has a NaN(Not-a-Number) element(s).
         */
        template <StrictArithmetic S>
        [[nodiscard]] constexpr PromotedMat4x3<T, S> safeDiv(S scalar, Mat4x3 fallback = Mat4x3::zero()) const noexcept
            requires StrictArithmetic<T>;
         ...
         */
        template <StrictArithmetic S>
        [[nodiscard]] static constexpr PromotedMat4x3<T, S> safeDiv(const Mat4x3& mat, S scalar,
                                                                    Mat4x3 fallback = Mat4x3::zero()) noexcept
            requires StrictArithmetic<T>;
         ...
         */
        template <StrictArithmetic S>
        [[nodiscard]] constexpr PromotedMat4x3<T, S> tryDiv(S scalar, OperationStatus& status,
                                                            Mat4x3 fallback = Mat4x3::zero()) const noexcept
            requires StrictArithmetic<T>;
         ...
         * @note If @p scalar is zero (or below the epsilon threshold) or this matrix contains NaN elements,
         *       returns @p fallback.
         * @note Promotes the result to the wider type using @ref PromotedFloatMat4x3<T, S>.
         * @note Operation is restricted to numeric types via @ref StrictArithmetic.
         * @note Returns @ref fallback if attempting to divide by zero (or below the epsilon threshold), or if any
         *       operand contains NaN.
         * @note In the event of multiple failure conditions, data corruption (NaN) takes precedence over
         ...
         */
        template <StrictArithmetic S>
        [[nodiscard]] static constexpr PromotedMat4x3<T, S> tryDiv(const Mat4x3& mat, S scalar, OperationStatus& status,
                                                                   Mat4x3 fallback = Mat4x3::zero()) noexcept
            requires StrictArithmetic<T>;
         ...
         * @return True if at least one element is positive or negative infinity.
         */
        [[nodiscard]] static constexpr bool hasInf(const Mat4x3& matrix) noexcept;
         ...
         * @return True if at least one element is NaN.
         */
        [[nodiscard]] static constexpr bool hasNaN(const Mat4x3& matrix) noexcept;
         ...
         */
        friend std::ostream& operator<<(std::ostream& os, const Mat4x3& matrix)
        {
        ...
        /**
         * @brief A 4x3 matrix with all ones.
         *
        ...
        /**
         * @brief A 4x3 matrix with all zero.
         *
         * @note Constrained to @ref StrictArithmetic types.
         */
    ...
    /** @brief Template deduction guide for Mat4x3. */
