 ...
 *
 * @brief SSE2+ implementation for Simd256.
 *
 * @details 256-bit SIMD register wrapper for x64-based systems.
 *          Support for faster instruction in higher instruction sets like AVX2 or AVX512
 *          is checked inside implementation. Note, there is no distinction made between SSE3,
...
namespace falcon
{
    ...
    //
    //
    //     /// @brief Initialize a Simd256 with the supplied register.
    //     explicit constexpr Simd256(simd::internal::SSERegister_t<DataType> reg): _register(reg) {}
    //
    //
    //     /**
    ...
    //     ///+=+=+=+=+=+=+=+=+=+=+=+=+=
    //
    //     /// Perform a logical right shift on the 256-bit register composed of Byte values.
    //     static constexpr __m256i _mm_srl_epi8_custom(__m256i reg, uint32_t count) noexcept;
    //
    //
    //     /// Perform a arithmetic right shift on the 256-bit register composed of Quad Word (64-bit integral) values.
    //     static constexpr __m256i _mm_sra_epi64_custom(__m256i reg, uint32_t count) noexcept;
    //
    //
    //     /// @brief Perform a logical left shift on the 256-bit register composed of Byte values by a constant known
    //     ///        at compile time.
    //     /// @tparam Count The shift amount.
    //     template <uint32_t Count>
    //     static constexpr __m256i _mm_slli_epi8_custom(__m256i reg) noexcept;
    //
    //
    //     /// @brief Perform a arithmetic right shift on the 256-bit register composed of Quad Word (64-bit integral)
    //     /// values
    //     ///        by a constant known at compile time.
    //     /// @tparam Count The shift amount.
    //     template <uint32_t Count>
    //     static constexpr __m256i _mm_srai_epi64_custom(__m256i reg) noexcept;
    //
    //
    //     /// @brief Perform a logical left right on the 256-bit register composed of Byte values by a constant known
    //     ///        at compile time.
    //     /// @tparam Count The shift amount.
    //     template <uint32_t Count>
    //     static constexpr __m256i _mm_srli_epi8_custom(__m256i reg) noexcept;
    //
    //
    //     /// @brief Perform a arithmetic left right on the 256-bit register composed of Byte values by a constant known
    //     ///        at compile time.
    //     /// @tparam Count The shift amount.
    //     template <uint32_t Count>
    //     static constexpr __m256i _mm_srai_epi8_custom(__m256i reg) noexcept;
    //
    ...
    //     static constexpr __m256i _mm_unpacklo_saturated_custom(__m256i reg) noexcept;
    //
    //
    //     /// @brief Unpack the upper lanes of the integral register into the next power of 2 (16->32)
    //     ///        with sign saturation.
    //     /// @note This has no effect on the 64-bit integrals.
    //     static constexpr __m256i _mm_unpackhi_saturated_custom(__m256i reg) noexcept;
    };
